...
---------------------------------------------------------------------------------------------------
 ...
 Title   		: GetNonPaidPackagesForCompany
 Purpose  		: Get the non paid packages for the company
 Instructions 	: 

-------------------------------------------------------------------------------------------------*/

IF EXISTS(SELECT NAME FROM SYSOBJECTS WHERE NAME = 'GetNonPaidPackagesForCompany')
BEGIN
	DROP PROCEDURE GetNonPaidPackagesForCompany 
END
GO

CREATE PROCEDURE [dbo].[GetNonPaidPackagesForCompany]
...
where p.CompanyID = @CompanyID and p.PaymentStatus != 'Payment Successful' and p.DateDeactivated is null
